...
def onMayaDroppedPythonFile(*args, **kwargs):
    if sys.version_info.major < 3:
        cmds.confirmDialog(title='ERROR:', message="{{ cookiecutter.project_name }} requires Python version 3.6 and higher. Current Maya Python interpreter is not compatible. \n\nAborting.", button=['OK'],
                       defaultButton='OK')
        return
...
def _add_module():
    ...
    cmds.confirmDialog(
        title="{{ cookiecutter.project_name }} Installed",
        message="{{ cookiecutter.project_name }} installed. Please restart Maya to see the shelf and menu items."
    )